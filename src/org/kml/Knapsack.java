...
public class Knapsack {
    public static int findMaxValue(int capacity, int[] weights, int[] values, int n) {
        ...
        if (weights[n - 1] <= capacity)
            return Math.max(
                    values[n - 1] + findMaxValue(capacity - weights[n - 1], weights, values, n - 1),
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 1; ++i) {
            ...
            int result = findMaxValue(capacity[i], weights[i], values[i], n[i]);
            System.out.println("\nThe maximum value we can earn is: " + result);
